...
$cookie_name = "viewed_" . $table . "_" . $num;
...
if ($table == "_qna") {
    ...
    <div class="ripple_box">
        <form name="ripple_form" method="post"
              action="insert_ripple.php?table=<?= $table ?>&num=<?= $num ?>&page=<?= $page ?>">
            <div class="ripple_box1"><img src="../img/ripple_title.png"></div>
            <div class="ripple_box2"><textarea name="ripple_content"></textarea></div>
            <div class="ripple_box3"><a href="#"><img src="../img/ripple_button.png" onclick="ripple_check_input()"></a>
            </div>
        </form>
    </div>
    <?php
...
<ul class="buttons">
    ...
    if ($userlevel == 1 or $userid == $id) {
        ...
        <li>
            <button onclick="location.href='<?= $delete_url ?>'">삭제하기</button>
        </li>
        <?php
    }
    ?>
    ...
    if ($userlevel == 1 or $table == "_youtube" or $table == "_qna") {
        ?>
        <li>
            <button onclick="location.href='<?= $write_url ?>'">글쓰기</button>
        </li>
        <?php
    }
    ?>
